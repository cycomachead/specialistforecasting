 ...
 * @since 180.store
 */
trigger SpecialistForecastTrigger on SpecialistForecast__c bulk(before insert, before update) {
	...
	if(Trigger.isBefore) {
		if(Trigger.isInsert || Trigger.isUpdate) {
			...
			for(SpecialistForecast__c specForecast : Trigger.new) {
				...
				monthAndFiscalYear = ForecastHelper.extractFYAndMonthFromDate(specForecast.ForecastCloseDate__c);
				specForecast.FYMonth__c = monthAndFiscalYear[0];
				specForecast.FiscalYear__c = monthAndFiscalYear[1];
				if(specForecast.ForecastAmount__c == null || specForecast.ForecastAmount__c < 0) {
					TextUtil.writeDebug('Invalid ForecastAmount__c: ' + specForecast.ForecastAmount__c);
