...
/**
 * Automation Test on Specialist Forecast Report User Drill Down
 * @author bob
 *
 */
@Listeners(SalesforceEnforcer.class)
public class SpecialistForecastAutomationUserDrillDownModel extends BaseTestCase{
	 /* Initializing logger */
    private static Logger logger = LoggerFactory.getLogger(SpecialistForecastAutomationUserDrillDownModel.class); 
     ...
     */
    //@Test(groups = "unit")
    public void specForecastReportUserDrillDownTest(){  
		...
		//specPage.enterData(LoginPage.getDevUser(), O2QUtil.getCurrentFiscalYear(), O2QUtil.getPreviousQuarter());
		//specPage.clickViewReport();
		specPage.clickUserDrillDown();
		checkReportHeader();
		checkUser();
    ...
    private void checkUser(){
		...
		//check cicked user drill down
		Assert.assertTrue(controller.getTableCellAllUser(driver, tableLocatorId).contains(specPage.getDefaultUser()));
